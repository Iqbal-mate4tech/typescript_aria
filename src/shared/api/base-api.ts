...
import { apiUrl } from '../constants'; // Adjust the path based on your project structure
...
const defaultOptions: RequestOptions = {
  ...
  headers: {
    'Content-Type': 'application/json',
    'ngrok-skip-browser-warning': 'true', // Can be commented as required
  },
  credentials: 'include',
};

export const makeRequest = async <T = any>(uri: string, options: RequestOptions = defaultOptions): Promise<T> => {
  if (uri !== apiUrl.userLogin) {
    options.headers = {
      ...options.headers,
  ...
  options = JSON.parse(JSON.stringify(options)); // Ensure no reference issues
...
export const makeOdooRequest = async <T = any>(uri: string, method: 'GET' | 'POST' = 'GET'): Promise<T> => {
  const requestOptions: RequestOptions = {
    method,
    redirect: 'follow',
  };
...
return await response.json();
    ...
    console.error('Odoo Request Error:', error);
    throw error;
  }
};

export const makeOdooPostRequest = async <T = any>(uri: string, data: any): Promise<T> => {
  const requestOptions: RequestOptions = {
    method: 'POST',
    body: JSON.stringify(data),
    redirect: 'follow',
...
export const doGetRequest = <T = any>(uri: string, options: RequestOptions = defaultOptions): Promise<T> => {
  options.method = 'GET';
  options.body = null;
  
return makeRequest<T>(uri, options);
};

export const doPostRequest = <T = any>(uri: string, data: any, options: RequestOptions = defaultOptions): Promise<T> => {
  options.method = 'POST';
...
export const doPutRequest = <T = any>(uri: string, data: any, options: RequestOptions = defaultOptions): Promise<T> => {
  options.method = 'PUT';
  options.body = JSON.stringify(data);
...
export const doDeleteRequest = <T = any>(uri: string, data: any, options: RequestOptions = defaultOptions): Promise<T> => {
...
export const doOdooGetRequest = <T = any>(uri: string, options: RequestOptions = defaultOptions): Promise<T> => {
  options.method = 'GET';
  options.body = null;
  
return makeOdooRequest<T>(uri);
};

export const doOdooPostRequest = <T = any>(uri: string, data: any, options: RequestOptions = defaultOptions): Promise<T> => {
  options.method = 'POST';
  options.body = JSON.stringify(data);
  
return makeOdooPostRequest<T>(uri,options);
};
